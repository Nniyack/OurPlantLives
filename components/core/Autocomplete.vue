<template>
  <div ref="autocompleteField" class="w-full">
    <div
      class="bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none rounded-md focus:ring-1"
    >
      <label class="relative block">
        ...
        <input
          type="text"
          :name="nameProps"
          class="pr-9 px-3 py-2 bg-white placeholder-slate-400 focus:outline-none block rounded-md sm:text-sm w-full"
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props: any, context: any) {
      ...
      onClickOutside(optionsList, () => (openListResultsSearch.value = false));
